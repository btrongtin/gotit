...
router.post('/', verifyToken, async (req, res) => {
	try {
		...
		if(result.remind){
			const date = new Date(result.remindAt);
			const user = await User.findOne({ uid: req.uid }).lean()
			const job = schedule.scheduleJob(result._id.toString(), date, function(){
				mailer.sendMail(user.email, "Reminds from GotIt", `<!doctype html>
				<html>
				  <head>
				...
				.ExternalClass div {
				  ...
				  <body style="background-color: #f6f6f6; font-family: sans-serif; -webkit-font-smoothing: antialiased; font-size: 14px; line-height: 1.4; margin: 0; padding: 0; -ms-text-size-adjust: 100%; -webkit-text-size-adjust: 100%;">
					<span class="preheader" style="color: transparent; display: none; height: 0; max-height: 0; max-width: 0; opacity: 0; overflow: hidden; mso-hide: all; visibility: hidden; width: 0;">You have a task to do today.</span>
								  ...
								  <table role="presentation" border="0" cellpadding="0" cellspacing="0" style="border-collapse: separate; mso-table-lspace: 0pt; mso-table-rspace: 0pt; width: 100%;" width="100%">
									<tr>
									  <td style="font-family: sans-serif; font-size: 14px; vertical-align: top;" valign="top">
										...
										<p style="font-family: sans-serif; font-size: 14px; font-weight: normal; margin: 0; margin-bottom: 15px;">Hope you have a nice day. I'm here to remind that you have a task to do today named <b>${result.name}</b>.</p>
										...
										<p style="font-family: sans-serif; font-size: 14px; font-weight: normal; margin: 0; margin-bottom: 15px;">Keep going and achieve your goals.</p>
		...
		// console.log('JOB CREATE: ', schedule.scheduledJobs[idJob])
		// schedule.scheduledJobs[idJob].reschedule()
...
router.put('/:id', verifyToken,async (req, res) => {
	...
	try {
		...
		if(result.oldCardRemind !== result.card.remind){
